...
public class ListTypeGenerator implements ObjectGenerator {
    ...
    private static final Logger LOG           = LogManager.getLogger(ListTypeGenerator.class);
    ...
    public Class<?> getItemClass(Type collectionItemType, Class<?> clz) {
        ...
        Assert.assertNotNull( genericType, "Failed to parse type!");

        Type[] typeArguments = genericType.getActualTypeArguments();

        Assert.assertEquals(typeArguments.length, 1, "Failed to parse, typeArguments.length = " + typeArguments.length);
