...
$(document).ready(function(){
	...
	$("#btn_passAcademic").click(function(){
		...
		if (row==null){
			...
			var url=path+"/review/passAcademicCollege";
		    var postData={
	...
	$("#btn_passAcademic_submit").click(function(){
	    ...
	    var url=path+"/review/refuseAcademicCollege";
	    var postData={
	    		"academicId":row.academicId,
	    		"memo":reasons
	    };
	...
	function initAcademic_detail(){
		var url=path+"/statistics/queryAcademicStatistics";
	 ...
	 /*******************导出excel 开始*****************/
	$('#btn_openexl_academic').click(function () {
		var url=path + "/statistics/academicExcel";
		var queryParams=$('#academic_queryForm').serializeJSON();
	...
	function setBtnStatus(row){
		...
		if (recommend=="1"){
			$("#btn_recommend_academic").linkbutton({"text":"取消推荐"});	
		}else{
			$("#btn_recommend_academic").linkbutton({"text":"推荐作品"});	
		}
		
	}
	//推荐
	$('#btn_recommend_academic').click(function(){
		var row = $('#tbl_academic_detail').datagrid('getSelected');
		 if (row==null){
			 alert("尚未选中任何记录！");
		 }else{	
			...
			var url=path+"/statistics/updAcademicRend ";
			  var postData={
			    		"academicId":row.academicId,
			    		"recommend":recommend
			    };
	...
	//取消推荐
	$('#btn_norecommend_academic').click(function(){
		var row = $('#tbl_academic_detail').datagrid('getSelected');
		 if (row==null){
			 alert("尚未选中任何记录！");
		 }else{
